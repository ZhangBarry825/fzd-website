...
<script>
  ...
  import {deleteApplication, getApplicationList, updateApplication} from "@/api/admin-application";
    ...
    methods: {
      ...
      deleteItems(){
        ...
        this.$confirm('This operation will permanently delete the data, continue?', 'Warn', {
          ...
          deleteApplication(formData).then(res=>{
      ...
      switchState(data){
        ...
        updateApplication(formData).then(res=>{
          console.log(res,876)
          if(res.code && res.code === 200){
            that.loading=false
          }
      ...
      handleClick(row) {
        this.goTo('/admin-application/edit?id='+row.id)
        console.log(row);
      },
